...
def apply_rating_operations(ratings):
    for i in range(len(ratings)-1):
        if ratings[i] == ratings[i+1]:
            ...
            ratings[i+1] = 0
    write_index = 0
    for read_index in range(len(ratings)):
        if ratings[read_index] != 0:
            ratings[write_index] = ratings[read_index]
            write_index += 1
    for i in range(write_index, len(ratings)):
        ratings[i] = 0
    return ratings
...
You are allowed to perform operations on this watchlist. In one operation, 
you can replace a show in watchlist with another show (i.e., another lowercase English letter).
...
3. While the left pointer is less than the right pointer:
   ...
   b. If the characters are different:
      * Replace the character that is alphabetically later (greater) with the one that is earlier (smaller) 
...
def make_smallest_watchlist(watchlist):
    new_watchlist = list(watchlist)
    left, right = 0, len(new_watchlist) - 1
    while left < right:
        if new_watchlist[left] != new_watchlist[right]:
            smaller = min(new_watchlist[left], new_watchlist[right])
            new_watchlist[left] = smaller
            new_watchlist[right] = smaller
        left += 1
        right -= 1
    return "".join(new_watchlist)
